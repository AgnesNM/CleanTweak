    ...
    let result = fetch(`${url}&apikey=${apiKey}`);   
       result.then(response => {
        return response.json();
         ...
         Object.values(userInputVal).forEach(value => {  
            console.log(value);
            
            // = {
            //     let organic: [,,,,{ title },,,,,]}}) = value; 
            //     console.log(title);
            // }               
...
//     const [data] = organic;
//     const{position, videos} = data;
//     const {title, url,parsed} = videos;
//     const{source,date,channel} = parsed;
